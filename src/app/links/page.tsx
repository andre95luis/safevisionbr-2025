...
import { Phone, Mail, Star, ExternalLink } from "lucide-react";

export const metadata: Metadata = {
  title: "SafeVisionBR — Links",
  description:
    "Contatos, redes sociais e avaliações da SafeVisionBR. Segurança eletrônica, WiFi e automação em São Paulo.",
  robots: { index: false },
};
...
// ⚠️ Atualize o link do Google Reviews com o ID da sua empresa no Google Maps
...
const links = [
  {
    label: "WhatsApp",
    sublabel: "Fale com a gente agora",
    href: WHATSAPP,
  ...
  {
    label: "Instagram",
    sublabel: "@safevisionbr · Projetos e novidades",
  ...
  },
  {
    ...
    sublabel: "⭐⭐⭐⭐⭐ · Veja o que falam de nós",
    href: GOOGLE_REVIEWS,
...
export default function LinksPage() {
  return (
    <main className="min-h-screen bg-slate-950 flex flex-col items-center px-4 py-12">
      <div className="w-full max-w-sm flex flex-col items-center gap-6">
        ...
        <div className="flex flex-col items-center gap-3 mb-2">
          <Image
            ...
            priority
          />
          <p className="text-slate-500 text-sm">@safevisionbr</p>
          <p className="text-slate-400 text-sm text-center leading-relaxed">
            Segurança eletrônica, WiFi e automação em São Paulo
          </p>
        </div>

        {/* Links */}
        <div className="w-full flex flex-col gap-3">
          {links.map(({ label, sublabel, href, border, bg, iconBg, labelColor, icon }) => (
            <a
              key={href}
              href={href}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`font-semibold text-sm truncate ${labelColor}`}>{label}</p>
        ...
        <div className="mt-4 flex flex-col items-center gap-1">
          ...
          <p className="text-slate-700 text-xs">São Paulo · SP</p>
        </div>
